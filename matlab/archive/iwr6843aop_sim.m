...
waveform = create_lfm_pulse_samples(Causal=1,ChirpUpDown=1,Fs=radar.Fs_hz,NumberOfSamples=n_samples);

% calculate maximum detectable range of radar

% calculate minimum target detection range of radar

% calculate the maximum unambiguous velocity of radar

% calculate the targets signal to noise ration(SNR)

% calculate target doppler frequency, amibuous doppler frequency, and
% apparent range rate(i.e. velocity) of target

% create linear FMCW waveform

% create radar datacube with range walk effects

% remove range walk effects from datacube
...
RDM = abs(sig_fft2);
RDM = mag2db(RDM);
[temp, index] = max(RDM, [], "all");
[x_idx, y_idx] = ind2sub(size(RDM),index);
